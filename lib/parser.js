...
* @Last Modified time: 2017-06-26 18:22:36
*/
...
class Parser {
    ...
    parse_func_stat() {
        ...
        */
        ...
        this.match(Token.LPARENT);
        let params = this.parse_parlist();
        this.match(Token.RPARENT);
    ...
    parse_for_stat() {
        ...
        if (this.cur_token.value == '=') {
            ...
            let stop = 1;
            if (this.cur_token.value == ',') {
                stop = this.parse_exp1();
            }

            let body = this.parse_forbody();
            this.match(Token.KEYWORD, "end");
            return this.ast.fornum_stat(name, begin, end, stop, body);
    ...
    parse_forbody() {
        ...
        this.match(Token.KEYWORD, "do");
        let block = this.parse_block();
        this.match(Token.KEYWORD, "block");
    ...
    parse_repeat_stat() {
        ...
        let cond = this.parse_block();
        this.match(Token.KEYWORD, "until");
        let block = this.parse_cond();

        return this.ast.repeat_stat(cond, block);
    }
    ...
    parse_expr_stat() {
        ...
        if (primaryexp.type == 'CallExpression') {
            ...
            assignment  = this.parse_assignment([primaryexp]);
    ...
    parse_subexpr(min_prec) {
        ...
        while (this.is_binary() && OP[this.cur_token.value].prec >= min_prec) {
            ...
            expr = this.ast.binary_expr(token.value, expr, rval);
        }
        ...
        if (this.cur_token.type == Token.NUMBER
            ...
            switch(this.cur_token.value) {
                ...
                case 'function':
                    ...
                    return this.ast.func_stat(null, params, block);
                default:
                    this.croak();
            }
        } else {
        ...
        let property;
         ...
         while (true) {
            switch(this.cur_token.value) {
                case '.':
                    this.next();
                    property = this.ast.identifier(this.match(Token.IDENTIFIER).name);
                    perfixexp = this.ast.member_expr(prefixexp, ".", property);
                    break;
                case '[':
                    this.next();
    ...
    parse_fieldsep() {
        ...
        if (this.cur_token.value == ','
            || this.cur_token.value == ';') {
            let cur_token = this.next();
            return this.literal(cur_token.value);
        }
    ...
    parse_filed() {
        ...
        if (this.cur_token.value == '[' || this.cur_token.type == Token.STRING) {
    ...
    parse_recfield() {
        ...
        return this.ast.listfield(key, value);
    }
    ...
    is_param() {
        if (this.cur_token.type == Token.STRING) {
            return true;
        }
