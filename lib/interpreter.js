...
* @Last Modified time: 2017-07-11 22:23:30
*/
...
class Interpreter {
    ...
    evaluate(node, env) {
        switch(node.type) {
            ...
            case 'Identifier':
                return this.evaulate_identifier(node, env);
            case 'Literal':
                return this.evaulate_literal(node, env);
            case 'AssignExpression':
                return this.evaulate_assign_expression(node, env);
            case 'LocalStatement':
    ...
    evaulate_literal(node, env) {
        return node.value;
    }

    evaulate_assign_expression(node, env) {
    ...
    evaluate_do_statment(node, env) {
        return this.evaulate(node.bdoy, env.extend());
    }
